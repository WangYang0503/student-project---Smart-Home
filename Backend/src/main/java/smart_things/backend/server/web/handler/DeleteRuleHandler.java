...
/**
 * This implementation of a http request handler is called when there are incoming "/delete" requests with which the
 * user wants to delete a rule from his Smart Home. In order to delete a rule it is necessary that the parameter "name"
 * (name of the rule as string) is passed as post parameter and that a rule with this name exists.
...
public class DeleteRuleHandler extends AbstractHandler {
     ...
     */
    public DeleteRuleHandler(DBManager dbManager, RuleWorker ruleWorker) {
